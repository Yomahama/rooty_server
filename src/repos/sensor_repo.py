...
class SensorRepository:
    def save(self, data: SensorDataIn, timestamp: str) -> None:
        conn = get_connection()
        conn.execute(
            "INSERT INTO measurements (lux, temperature, moisture, battery, humidity, soil_temp, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (data.lux, data.temperature, data.moisture, data.battery, data.humidity, data.soil_temp, timestamp),
        )
        conn.commit()
        conn.close()
