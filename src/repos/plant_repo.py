...
class PlantRepository:
    def get_all(self) -> list[Plant]:
        conn = get_connection()
        rows = conn.execute("SELECT id, name, dli_min, dli_max, temp_min, temp_max, moisture_min, moisture_max FROM plants").fetchall()
        conn.close()
    ...
    def get_by_id(self, plant_id: int) -> Plant | None:
        conn = get_connection()
        row = conn.execute("SELECT id, name, dli_min, dli_max, temp_min, temp_max, moisture_min, moisture_max FROM plants WHERE id = ?", (plant_id,)).fetchone()
        conn.close()
        if not row:
            return None
